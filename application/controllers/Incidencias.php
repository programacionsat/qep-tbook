...
*/
        ...
        // Control para las incidencias sin servicio afectado y otros servicios
        // ya que sus umbrales no están completos. Vamos rellenando con 0 las 
        // horas para las que no haya datos (hasta llegar a la hora actual)

        // [ToDo] esto habría que solucionarlo en base de datos, en la tabla
        //  *qep-tbook.umbrales_servicios*
        for ($h = 0; $h <= date("G"); $h++) {
            if (!array_key_exists($h, $datos_umbrales_servicio["sin"])) {
                $datos_umbrales_servicio["sin"][$h] = 0;
            }

            if (!array_key_exists($h, $datos_umbrales_servicio["otros"])) {
                $datos_umbrales_servicio["otros"][$h] = 0;
            }
        }
